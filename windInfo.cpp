...
namespace zlib
{
	namespace window
	{
		void windowObject::updateScaledDisplacement()
		{
			scaledDisplacement = scale(displacement, false).negatedY();
		}



		void windowObject::setDisplacement(var::coord2 _displacement)
		{
			displacement = _displacement;
			updateScaledDisplacement();
		}

		void windowObject::incrimentDisplacement(double x, double y)
		{
			displacement += var::coord2(x, y);
			updateScaledDisplacement();
		}

		void windowObject::incrimentDisplacement(var::coord2 incriment)
		{
		...
		var::coord2 windowObject::getDisplacement() { return displacement; }
		var::coord2 windowObject::getScaledDisplacement() { return scaledDisplacement; }


		
		void windowObject::setScaleTotal(double _scaleTotal)
		{
			if(_scaleTotal <= 0) throw "_scaleTotal cannot be less than or equal to 0";
			else scaleTotal = _scaleTotal;
		}

		double windowObject::getScaleTotal() { return scaleTotal; }
		...
		double windowObject::scaleX(double coord)
		{
			return coord * scaleMultiplier;
		}

		double windowObject::scaleY(double coord, bool yFlip)
		{
		...
		double windowObject::unscaleX(double coord)
		{
			return coord / scaleMultiplier;
		}

		double windowObject::unscaleY(double coord, bool yFlip)
		{
			double ret = coord;
		...
		var::coord2 windowObject::scale(var::coord2 coord, bool yFlip)
		{
			return var::coord2(scaleX(coord.x), scaleY(coord.y, yFlip));
		}

		var::coord2 windowObject::unscale(var::coord2 coord, bool yFlip)
		{
		...
		var::coord2 windowObject::getRotation(float degrees, var::coord2 rotationPointCoords, var::coord2 coords, bool debug)
		{
